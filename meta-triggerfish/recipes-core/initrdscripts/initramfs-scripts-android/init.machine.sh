...
if [ ! -e $BOOT_DIR/etc/asteroid/machine.conf ] ; then
    ...
    # Each fossil gen 5 has its own unique product id, but we just use the generic triggerfish.
    echo triggerfish > /sys/kernel/config/usb_gadget/g1/strings/0x409/product
